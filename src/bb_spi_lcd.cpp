...
int spilcdInit(SPILCD *pLCD, int iType, int iFlags, int32_t iSPIFreq, int iCS, int iDC, int iReset, int iLED, int iMISOPin, int iMOSIPin, int iCLKPin)
{
	...
	if (pLCD->iLEDPin != -1)
    {
        ...
        myPinWrite(pLCD->iLEDPin, HIGH); // turn on the backlight
    }
	if (pLCD->iResetPin != -1)
	{
        pinMode(pLCD->iResetPin, OUTPUT);
		myPinWrite(pLCD->iResetPin, HIGH);
		delayMicroseconds(60000);
		myPinWrite(pLCD->iResetPin, LOW); // reset the controller
		delayMicroseconds(60000);
		myPinWrite(pLCD->iResetPin, HIGH);
		delayMicroseconds(60000);
	}
...
void spilcdShutdown(SPILCD *pLCD)
{
    ...
    myPinWrite(pLCD->iLEDPin, LOW); // turn off the backlight
